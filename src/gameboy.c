...
void dump_memory(gameboy *gb)
{
    ...
    if (dumpfile == NULL)
    {
        fprintf(stderr, "DEBUG NOTE: Error opening Game Boy memory dumpfile (%s)\n", dumpfile_path);
        return;
    }
    ...
    {
        fprintf(stderr, "DEBUG NOTE: Error dumping Game Boy memory into %s\n", dumpfile_path);
    }
    else
    {
        printf("DEBUG NOTE: Game Boy memory dumped to %s\n", dumpfile_path);
    }
